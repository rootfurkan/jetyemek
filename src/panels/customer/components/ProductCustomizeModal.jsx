...
export default function ProductCustomizeModal({ isOpen, onClose, product, restaurant, onAddToCart, addToast }) {
  ...
  // Derived Values
  const isBurger = product.category === 'Burgerler' || product.name.toLowerCase().includes('burger');
  const isPizza = product.category === 'Pizzalar' || product.name.toLowerCase().includes('pizza');
  const isDessert = product.category === 'Tatlılar' || product.name.toLowerCase().includes('tatlı');
  ...
  const handleMultiSelect = (groupId, choice) => {
    setSelectedMultiOptions(prev => {
      ...
      if (exists) {
        return { ...prev, [groupId]: current.filter(c => c.name !== choice.name) };
      } else {
  ...
  const handleAddToCartClick = () => {
    ...
    for (const group of optionGroups) {
      if (group.type === 'single' && group.required) {
        if (!selectedSingleOptions[group.id]) {
          addToast({ message: `Lütfen zorunlu bir seçim yapın: ${group.title}`, type: 'warning' });
          return;
        }
      }
    }
    ...
    const finalId = selectedNames.length > 0 
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-end sm:items-center justify-center p-0 sm:p-4">
          ...
          >
            ...
            {product.image && (
              <div className="w-full h-48 sm:h-64 bg-stone-100 relative shrink-0">
            ...
            <div className="flex-1 overflow-y-auto pb-24 text-left">
              <div className="p-6">
                <h2 className="text-2xl font-black text-stone-800 leading-tight">{product.name}</h2>
                <p className="text-xl font-extrabold text-primary mt-1">₺{product.price}</p>
                <p className="text-sm text-stone-500 font-medium mt-3 leading-relaxed">
                  {product.description || 'Restoranın özenle hazırladığı nefis lezzet.'}
                </p>
              </div>
              ...
              <div className="px-6 space-y-6">
                ...
                {optionGroups.map(group => (
                  <div key={group.id} className="bg-stone-50 border border-stone-100 rounded-2xl p-4 sm:p-5">
                    <div className="flex justify-between items-center mb-4">
                      <div>
                        <h4 className="font-bold text-stone-800 text-base">{group.title}</h4>
                        <span className="text-[10px] text-stone-500 font-bold uppercase tracking-wider bg-stone-200/50 px-2 py-0.5 rounded-full mt-1 inline-block">
                          {group.type === 'single' ? (group.required ? 'Zorunlu Seçim (1)' : 'İsteğe Bağlı (En fazla 1)') : 'İsteğe Bağlı'}
                        </span>
                      </div>
                    </div>
                    ...
                    <div className="space-y-3">
                      {group.choices.map((choice, idx) => {
                        ...
                        const isSelected = isSingle 
                          ? selectedSingleOptions[group.id]?.name === choice.name
                          : selectedMultiOptions[group.id]?.some(c => c.name === choice.name);

                        return (
                          <label key={idx} onClick={() => isSingle ? handleSingleSelect(group.id, choice) : handleMultiSelect(group.id, choice)} className={`flex items-center justify-between py-3 px-4 rounded-xl border transition-all cursor-pointer group ${isSelected ? 'border-primary bg-primary/5 text-primary shadow-sm shadow-primary/10' : 'border-stone-200 text-stone-600 hover:bg-stone-50'}`}>
                            <div className="flex items-center gap-3">
                              <div className={`w-5 h-5 flex items-center justify-center border-2 transition-all ${isSingle ? 'rounded-full' : 'rounded'} ${isSelected ? 'border-primary bg-primary' : 'border-stone-300 bg-white group-hover:border-primary/50'}`}>
                                {isSelected && (
                                  <span className="material-symbols-outlined text-[14px] text-white font-extrabold" style={{ fontVariationSettings: "'FILL' 1" }}>
                                    {isSingle ? 'circle' : 'check'}
                                  </span>
                                )}
                              </div>
                              <span className={`text-sm font-semibold ${isSelected ? 'text-stone-800' : 'text-stone-600 group-hover:text-stone-800'}`}>
                                {choice.name}
                              </span>
                            </div>
                            {choice.price > 0 && (
                              <span className={`text-xs font-extrabold px-2 py-1 rounded-lg ${isSelected ? 'text-primary bg-white border border-primary/20' : 'text-stone-500 bg-white border border-stone-200'}`}>
                                +{choice.price} TL
                              </span>
                            )}
                          </label>
                ...
                <div className="bg-stone-50 border border-stone-100 rounded-2xl p-4 sm:p-5">
                  <div className="mb-3">
                    <h4 className="font-bold text-stone-800 text-base">Ürün Notu</h4>
                    <span className="text-[10px] text-stone-500 font-bold uppercase tracking-wider bg-stone-200/50 px-2 py-0.5 rounded-full mt-1 inline-block">
            ...
            <div className="absolute bottom-0 inset-x-0 bg-white border-t border-stone-100 p-4 sm:p-5 flex flex-col sm:flex-row items-center gap-4 shadow-[0_-10px_30px_rgba(0,0,0,0.05)] z-20">
              <div className="flex items-center justify-between bg-stone-100 rounded-xl p-1 w-full sm:w-auto shrink-0">
                ...
                <span className="w-10 text-center font-black text-stone-800 text-base">{quantity}</span>
                <button
