...
import CreditCardForm, {
  validateCardForm,
} from "../../../common/components/CreditCardForm.jsx";
...
function getOrderTrackingStatus(order) {
  if (order.deliveryStatus === "ready") {
    return {
      key: "ready",
      label: "Siparis Hazir",
      progress: order.progress ?? 40,
    };
  }
  if (order.deliveryStatus === "on_the_way") {
    return {
      key: "on_the_way",
      label: "Kurye Yola Cikti",
      progress: order.progress ?? 65,
    };
  }
...
function ActiveOrderTracker({ order, restaurantName, onStatusUpdate }) {
  ...
  const currentStepIndex = Math.max(
    0,
    statusSteps.findIndex((s) => s.key === trackingStatus.key),
  );
      ...
      <div className="bg-gradient-to-r from-rose-600 to-red-500 px-6 py-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-rose-100 text-[11px] font-medium">
              {restaurantName}
            </p>
          </div>
        </div>
...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    async function fetchOrders(showLoader = false) {
      ...
      try {
        ...
        if (ordersSnapshotRef.current !== snapshot) {
    ...
    fetchOrders(true);
    const refreshInterval = setInterval(() => fetchOrders(false), 3000);
  ...
  useEffect(() => {
    ...
    if (activeOrder.status === "Hazırlanıyor") {
      ...
      timerRef.current = setTimeout(async () => {
        ...
        addToast({
          message: "🛵 Kurye siparişinizi aldı ve yola çıktı!",
          type: "success",
          duration: 5000,
        });
      }, 15000);
  ...
  const handleAddAddressBtn = async (e) => {
    ...
    if (!newAddrTitle.trim() || !newAddrDetails.trim()) {
      addToast({
        message: "Lütfen adres başlığı ve detayını boş bırakmayın!",
        type: "error",
      });
      return;
    }
  ...
  const handleAddCardBtn = (e) => {
    ...
    if (!isValid) {
      ...
      addToast({
        message: "Lütfen kart bilgilerini eksiksiz ve doğru doldurun.",
        type: "error",
      });
      return;
    }
    ...
    const cardLogos = {
      ...
      Mastercard:
        "https://lh3.googleusercontent.com/aida-public/AB6AXuCepLVd8hsizjFucB5HUGRh9P5WIbXle5yDNhxbFZ4IUL4x-UUkzX8Twd9ThLtYulSUXTlhtuaaVFRS4E8y5h0Ced3Fz_jI3E5m4xVrkEaQF6VNkeccJLSAtlN4ITJwO_hYI8F-o-V5HRmx33xv3iuacoJjXQWrraAK8fMmgFSeJPme2Oz95nnutZMot7FnWfo_9W0yzrvN_Goq-eetI761mTfWrpRY5le3T5J84fwXs1hlITDhkgaqKA",
    ...
    addToast({
      message: "Yeni ödeme yönteminiz başarıyla eklendi!",
      type: "success",
    });
  };
  ...
  const handleSavePreferences = () => {
    addToast({
      message: "Kampanya tercihleriniz başarıyla kaydedildi!",
      type: "success",
    });
  };
  ...
  const handleSubmitReview = async () => {
    ...
    const newReview = {
      ...
      user:
        currentUser.name +
        (currentUser.surname ? " " + currentUser.surname.charAt(0) + "." : ""),
      rating: reviewRating,
    ...
    try {
      ...
      addToast({
        message:
          savedReview.status === "Onay Bekliyor"
      ...
      <Sidebar
        ...
        onChangeAvatar={() => {
          const newUrl = window.prompt(
            "Lütfen yeni profil resminizin URL adresini giriniz:",
            userProfile.avatar,
          );
              ...
              {activeOrder &&
                !["delivered", "cancelled"].includes(
                  ...
                  <div>
                    ...
                    <ActiveOrderTracker
                      order={activeOrder}
                      restaurantName={getRestaurantName(activeOrder)}
                    />
                  </div>
                )}
            </AnimatePresence>
            ...
            <div>
              ...
              <div className="space-y-4">
                {ordersLoading ? (
                  ...
                  <div className="text-center py-12 bg-white rounded-2xl border border-stone-100">
                    <span className="material-symbols-outlined text-5xl text-stone-200">
                      receipt_long
                    </span>
                    <p className="text-stone-400 text-sm font-semibold mt-3">
                        ...
                        <div className="w-16 h-16 rounded-xl overflow-hidden shrink-0 border border-stone-100 bg-stone-50">
                          {order.image ? (
                            ...
                            <div className="w-full h-full flex items-center justify-center">
                              <span className="material-symbols-outlined text-stone-300 text-2xl">
                                restaurant
                              </span>
                            </div>
                          )}
                        ...
                        <div className="flex flex-col sm:flex-row gap-2 mt-2 sm:mt-0">
                          ...
                          <button
                            onClick={() => {
                              ...
                              addToast({
                                message: `${order.restaurant} siparişiniz sepetinize tekrar eklendi!`,
                                type: "success",
                              });
                              navigate("/cart");
                            }}
              ...
              <div className="space-y-4">
                {reviews.filter(
                  ...
                  <div className="text-center py-12 bg-stone-50 rounded-2xl border border-stone-100">
                    ...
                    <p className="text-stone-500 text-sm font-semibold mt-3">
                  ...
                  reviews
                    ...
                    .map((review) => (
                      ...
                      >
                        <div className="flex justify-between items-start mb-2">
                          <div>
                            <h4 className="font-bold text-stone-800 text-sm">
                              {restaurants.find(
                                (res) => res.id === review.restaurantId,
                              )?.name || "Restoran"}
                            </h4>
                            <p className="text-[10px] text-stone-400 font-semibold mt-0.5">
                              {review.date}
                        ...
                        <p className="text-stone-600 text-xs font-medium leading-relaxed mt-2">
                          {review.comment}
                        </p>
                        ...
                        {review.reply && (
                          <div className="mt-4 bg-white p-4 rounded-xl border-l-4 border-primary shadow-sm">
                            ...
                            <p className="text-stone-600 text-xs font-medium italic">
                              {review.reply}
                            </p>
                          </div>
                        )}
        ...
        {activeSubSection === "profile" && (
          <section className="space-y-6">
            <div className="bg-white rounded-3xl shadow-sm p-6 md:p-8 border border-stone-100 text-left">
              ...
              >
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-stone-500 ml-1">
                    Ad
                  </label>
                  <input
                    value={profileName}
            ...
            <div className="bg-white rounded-3xl shadow-sm p-6 md:p-8 border border-stone-100 text-left">
              ...
              {showAddressForm && (
                <form
                  onSubmit={handleAddAddressBtn}
                  className="bg-stone-50 p-5 rounded-2xl border border-stone-200/50 mb-6 space-y-4 text-left"
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                {addresses.map((addr) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="material-symbols-outlined text-stone-400 hover:text-rose-500 text-[16px] p-1 rounded-md hover:bg-stone-100 cursor-pointer border-none bg-transparent"
                    >
                      delete
                    </button>
                  </div>
                ...
                {addresses.length === 0 && (
                  <p className="text-xs text-stone-400 col-span-2 py-4 text-center">
        ...
        {activeSubSection === "cards" && (
          <section className="space-y-6">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 text-left">
              <div className="flex items-center gap-3">
                <span
                  className="material-symbols-outlined text-primary text-3xl select-none"
                  style={{ fontVariationSettings: "'FILL' 1" }}
                >
                  credit_card
                </span>
              ...
              <button
                ...
                className="flex items-center gap-1 px-6 py-2.5 bg-gradient-to-r from-primary to-secondary text-white rounded-full font-bold text-xs shadow-md shadow-primary/10 active:scale-95 transition-all cursor-pointer border-none"
              >
                <span className="material-symbols-outlined text-sm font-bold">
                  add
                </span>
            ...
            {showCardForm && (
              <form
                onSubmit={handleAddCardBtn}
                className="bg-white rounded-3xl p-6 border border-stone-100 shadow-sm text-left space-y-4"
              >
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      className="rounded-xl border border-stone-200/80 bg-stone-50 focus:border-primary focus:outline-none py-2 px-3 text-xs"
                    >
                  ...
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] uppercase tracking-wider text-stone-400 font-bold font-mono">
                          {card.type}
                        </p>
                      </div>
                    </div>
                    <button
                      ...
                      className="w-8 h-8 flex items-center justify-center rounded-full hover:bg-rose-50 hover:text-primary text-stone-400 transition-colors cursor-pointer border-none bg-transparent"
                    >
            ...
            <div className="bg-rose-50/20 border border-rose-100/40 rounded-3xl p-6 flex gap-4 items-start text-left">
              ...
              <div>
                ...
                <p className="text-[11px] text-stone-500 leading-relaxed max-w-2xl font-medium">
                  Kart bilgileriniz PCI-DSS uyumlu sunucularımızda en yüksek
                  güvenlik standartlarıyla şifrelenerek saklanmaktadır. JetYemek
                  personeli dahil hiç kimse tam kart numaranıza erişemez.
                </p>
              </div>
            </div>
          </section>
          ...
          <section className="space-y-6 text-left">
            <div className="bg-white rounded-3xl shadow-sm p-6 md:p-8 border border-stone-100">
              ...
              <p className="text-xs text-stone-400 font-medium mb-6">
                Size özel indirimlerden, yeni restoran keşiflerinden ve
                avantajlı kampanyalardan ilk siz haberdar olun.
              </p>
              <div className="space-y-4">
                <label className="flex items-start gap-4 bg-stone-50 hover:bg-stone-50/80 p-4 rounded-2xl border border-stone-100 transition-colors cursor-pointer select-none">
                  ...
                  <div>
                    <h5 className="font-bold text-stone-800 text-xs">
                      SMS ile indirim bilgisi almak istiyorum
                    </h5>
                    <p className="text-[10px] text-stone-400 font-semibold mt-0.5">
                      Önemli güncellemeleri ve kurye bilgilerini içeren anlık
                      mesajlar.
                    </p>
                  </div>
                </label>
                <label className="flex items-start gap-4 bg-stone-50 hover:bg-stone-50/80 p-4 rounded-2xl border border-stone-100 transition-colors cursor-pointer select-none">
                  ...
                  <div>
                    <h5 className="font-bold text-stone-800 text-xs">
                      E-posta ile indirim bilgisi almak istiyorum
                    </h5>
                    <p className="text-[10px] text-stone-400 font-semibold mt-0.5">
                      Haftalık bültenler, restoran önerileri ve kişiye özel
                      kupon kodları.
                    </p>
                  </div>
                </label>
            ...
            <div className="bg-white rounded-3xl shadow-sm p-6 md:p-8 border border-stone-100">
              ...
              <div className="p-4 bg-rose-50/20 border border-rose-100/30 rounded-2xl flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="text-left">
                  <h5 className="font-bold text-stone-800 text-xs">
                    Hesabı Tamamen Kapat
                  </h5>
                  <p className="text-[10px] text-stone-400 font-semibold mt-0.5 max-w-md">
                    Bu işlem geri alınamaz. Sipariş geçmişiniz ve kayıtlı
                    kartlarınız kalıcı olarak silinecektir.
                  </p>
                </div>
                <button
        ...
        <div className="p-4 sm:p-6 text-center space-y-6">
          <div>
            <h3 className="font-extrabold text-stone-800 text-lg">
              {getRestaurantName(reviewTargetOrder || {})}
            </h3>
            ...
            {[1, 2, 3, 4, 5].map((star) => (
              ...
              >
                <span
                  ...
                  style={{
                    fontVariationSettings:
                      reviewRating >= star ? "'FILL' 1" : "'FILL' 0",
                  }}
                >
                  star
                </span>
